...
class HOMExShapeletPair:
    def __init__(
        ...
        if not is_self_defined_PSF:
            ...
            if psf_type == "gaussian":
                ...
                self.psf_base = galsim.Kolmogorov(
                ...
                self.psf_base = self.toSize(
                    self.psf_base, self.psf_sigma, weighted=True
                )
    ...
    def setup_shapelet_psf(self, m, c, n, bmax=10):
        ...
        self.dm = (
            m
            * self.sxm.get_all_moments(self.sxm.base_psf_image, self.sxm.get_pq_full(n))
            + c
        )
    ...
    def toSize(self, profile, sigma, weighted=True, tol=1e-4):
        ...
        if weighted:
            ...
            true_sigma = (
                galsim.hsm.FindAdaptiveMom(
                    profile.drawImage(scale=apply_pixel, method="no_pixel")
                ).moments_sigma
                * apply_pixel
            )
        else:
        ...
        while abs(true_sigma - sigma) > tol:
            ...
            if weighted:
                ...
                true_sigma = (
                    galsim.hsm.FindAdaptiveMom(
                        new_profile.drawImage(scale=apply_pixel, method="no_pixel"),

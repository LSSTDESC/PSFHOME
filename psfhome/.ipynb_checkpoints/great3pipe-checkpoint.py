...
#     import .metasm
# except:
#     print('failed to import the script for metacalibration')
...
class GREAT3Pipe:
    def __init__(
        ...
        if not is_self_defined_PSF:
            ...
            if psf_type == "gaussian":
                ...
                self.psf_light = galsim.Kolmogorov(
                    flux=1.0, half_light_radius=self.psf_sigma
                )
                # self.psf_light = self.toSize(self.psf_light, self.psf_sigma)

                psf_model_light = 0.5 * self.psf_light + 0.5 * self.psf_light.expand(
                    mod_kol_radius_ratio
                )

                old_sigma = (
                    galsim.hsm.FindAdaptiveMom(
                        self.psf_light.drawImage(scale=self.pixel_scale)
                    ).moments_sigma
                    * self.pixel_scale
                )
                ...
                # print(ratio)
                self.psf_model_light = self.toSize(psf_model_light, old_sigma).withFlux(
                    1.0
                )
            elif psf_type == "moffat":
                self.psf_light = galsim.Moffat(beta0, half_light_radius=self.psf_sigma)
                self.psf_light = self.toSize(
                    self.psf_light, self.psf_sigma, weighted=True
                )
                self.psf_model_light = galsim.Moffat(
                    beta0 + delta_beta, half_light_radius=self.psf_sigma
                )
                self.psf_model_light = self.toSize(
                    self.psf_model_light, self.psf_sigma, weighted=True
                )
                ...
                self.psf_light = self.toSize(self.psf_light, self.psf_sigma)
                self.psf_model_light = galsim.Gaussian(
                    flux=1.0, sigma=self.psf_model_sigma
                )

            self.psf_light = self.psf_light.shear(e1=psf_e1, e2=psf_e2)
            self.psf_model_light = self.psf_model_light.shear(
                e1=self.psf_model_e1, e2=self.psf_model_e2
            )
        else:
        ...
        if self.shift:
            if rot:
                image = final.drawImage(
                    scale=self.pixel_scale, offset=(self.shift1_rot, self.shift2_rot)
                )
            else:
                image = final.drawImage(
                    scale=self.pixel_scale, offset=(self.shift1_ori, self.shift2_ori)
                )
        else:
    ...
    def toSize(self, profile, sigma, weighted=True, tol=1e-4):
        ...
        if weighted:
            ...
            true_sigma = (
                galsim.hsm.FindAdaptiveMom(
                    profile.drawImage(scale=apply_pixel)
                ).moments_sigma
                * apply_pixel
            )
        else:
        ...
        while abs(true_sigma - sigma) > tol:
            ...
            if weighted:
                ...
                true_sigma = (
                    galsim.hsm.FindAdaptiveMom(
                        new_profile.drawImage(scale=apply_pixel),
...
def df_block(params, index, metacal=True):
    ...
    try:
        ...
        truth_kurtosis = test.find_kurtosis_2(
            test.psf_light.drawImage(scale=test.pixel_scale)
        )
        model_kurtosis = test.find_kurtosis_2(
            test.psf_model_light.drawImage(scale=test.pixel_scale)
        )

...
class PSFSameSecondTest:
    ...
    def get_intersection(self, metacal):
        ...
        if metacal == False:
            ...
            intersection = (
                np.sqrt(results.corrected_e1**2 + results.corrected_e2**2)
                - self.e_truth
            )
            return intersection
        else:
            ...
            intersection = (
                np.sqrt(results["g_cal"].e1 ** 2 + results["g_cal"].e2 ** 2)
                - self.e_truth
            )
            return intersection
    ...
    def get_prediction(self):
        return self.e_truth * (
            self.psf_model_sigma**2 - self.psf_sigma**2
        ) / self.gal_sigma**2 + (self.psf_sigma / self.gal_sigma) ** 2 * (
            self.psf_e2 - self.psf_model_e2
        )
    ...
    def findAdaptiveSersic(self, sigma, n):
        good_half_light_re = bisect(
            Sersic_sigma, sigma / 3, sigma * 5, args=(n, self.pixel_scale, sigma)
        )
    ...
    def findAdaptiveOpticalPSF(self, sigma):
        good_fwhm = bisect(
            OpticalPSF_sigma,
            max(self.psf_sigma / 3, self.pixel_scale),
            self.psf_sigma * 5,
            args=(self.pixel_scale, sigma),
        )
    ...
    def toSize(self, profile, sigma):
        true_sigma = (
            galsim.hsm.FindAdaptiveMom(
                profile.drawImage(scale=self.pixel_scale, method="no_pixel")
            ).moments_sigma
            * self.pixel_scale
        )
    ...
    def get_results(self, metacal=False):
        ...
        results["psf_kurtosis"] = find_kurtosis_2(
            self.psf_light.drawImage(scale=self.pixel_scale)
        )
        results["psf_model_kurtosis"] = find_kurtosis_2(
            self.psf_model_light.drawImage(scale=self.pixel_scale)
        )
        results["kurtosis_perc_error"] = (
            100
            * (results["psf_model_kurtosis"] - results["psf_kurtosis"])
            / results["psf_model_kurtosis"]
        )

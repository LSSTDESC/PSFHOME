...
class Fisher:
    ...
    def _makeSourcePZ(self, file='fisher_data/nzdist.txt', implement_outliers=True, y1 = False):
    ...
    def buildCovMatrix(self,y1 = False):
        ...
        if self.calcCov:
            ...
            if y1==True:
                invcov_SRD = pd.read_csv('fisher_data/Y1_shear_shear_inv.txt', names=['a','b'], delimiter=' ')
            else:
                invcov_SRD = pd.read_csv('fisher_data/Y10_shear_shear_inv.txt', names=['a','b'], delimiter=' ')
    ...
    def getC_ellOfSigma8(self, sigma8):
        cosmo = ccl.Cosmology(Omega_c=self.vals['omega_m'] - self.vals['omega_b'], Omega_b=self.vals['omega_b'], h=self.vals['h'], sigma8=sigma8, n_s=self.vals['n_s'], 
            transfer_function='eisenstein_hu', matter_power_spectrum = self.cosmo._config_init_kwargs['matter_power_spectrum'])
